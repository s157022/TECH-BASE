...
//編集機能フォーム
	if(!empty($_POST["edit"])){
      ...
      try{
        $sql = 'SELECT * FROM mission_T WHERE id=:edit';
...
if(isset($_POST["comment"]) && empty($_POST["e_judge"])){
	if(!empty($_POST["pass"])){
		...
		try{
		$sql = $pdo -> prepare('INSERT INTO mission_T (name, comment, time, passwd) VALUES (:name, :comment, :time, :passwd)');
...
<?php
 //削除処理
	...
	$delete = $_POST["delete"];
	try{
		$sql = 'SELECT passwd FROM mission_T where id=:del_id';
		$stmt = $pdo->prepare($sql);
        ...
        if($del_pass == $check_pass["passwd"]){
            $sql = 'DELETE FROM mission_T WHERE id=:del_id';
            $stmt = $pdo->prepare($sql);
			$stmt->bindParam(':del_id', $delete, PDO::PARAM_INT);
            $stmt->execute();
             }else{  //パスワード不一致の場合
...
if(!empty($_POST["e_judge"])){  //編集作業であることを判定し、ファイルに再書き込み
	...
	try{
	$sql = 'UPDATE mission_T SET name=:new_name, comment=:new_comment, time=:new_time, passwd=:new_pass WHERE id=:num';
	$stmt = $pdo->prepare($sql);
